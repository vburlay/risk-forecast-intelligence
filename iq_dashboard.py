...
def build_survival_risk_df() -> pd.DataFrame:
    ...
    df["Expected Time to Gap"] = expected_days
...
def build_expected_time_gap_fig():
    ...
    plot_df["ExpectedTimeNum"] = plot_df["Expected Time to Gap"].map(mapping).fillna(31)
    plot_df["ExpectedTimeLabel"] = plot_df["Expected Time to Gap"].map(label_mapping).fillna(">30")
...
def survival_risk_grid_data():
    ...
    column_defs = [
        ...
        {"headerName": "Expected Time to Gap", "field": "Expected Time to Gap", "minWidth": 180, "flex": 1},
        {"headerName": "Risikostatus", "field": "Risikostatus", "minWidth": 140, "flex": 1, "cellStyle": risikostatus_cell_style()},
    ]
    return display_df.to_dict("records"), column_defs
...
- **Expected Time to Gap** = erwarteter Zeitraum bis zum nächsten kritischen Ereignis. 
